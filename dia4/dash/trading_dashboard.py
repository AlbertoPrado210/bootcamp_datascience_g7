...
def obtener_precios_cierre(simbolo: str) -> pd.DataFrame:
    ...
    try:
        ...
        return pd.DataFrame() # Devuelve vacío en caso de error de red o timeout
...
def actualizar_grafico(_, simbolo):
    ...
    fig = px.line(
        ...
        x="Date",
        y="Close",
        hover_data={"Variacion": ":+.2f"}, # Usamos la columna calculada para mostrarla en el tooltip
        markers=True,
        title=f"Precio de cierre de {simbolo} en el último mes",
        labels={"Date": "Fecha", "Close": "Precio de Cierre (USD)", "Variacion": "Variación vs Día 1 (USD)"},
    )
